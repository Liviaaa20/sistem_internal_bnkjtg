...
function hitungKPR() {
    ...
    if (!nominalAwal || nominalAwal < 50000000) {
        Toast.fire({ icon: 'warning', title: 'Minimal pinjaman Rp 50.000.000' });
        return;
    }
...
function openContactModal() {
    Swal.fire({
        ...
        html: `
            <input id="swal-Jabatan" class="swal2-input" placeholder="Jabatan" value="${document.getElementById('cpJabatan').innerText}">
        ...
        preConfirm: () => {
            return [
                document.getElementById('swal-Jabatan').value,
        ...
        if (result.isConfirmed) {
            document.getElementById('cpJabatan').innerText = result.value[0].toUpperCase();
            document.getElementById('cpNama').innerText = result.value[1];
            document.getElementById('cpPhone').innerText = result.value[2];
        }
    })
}
